                ...
                <div class="card-body border-bottom border-light">
                    ...
                    <fieldset class="mb-12">
                        <div class="form-group row">
                            ...
                            <div class="col-lg-10">
                                <input type="hidden" name="supplier_id" id="supplier_id" value="{{ $data->supplier_id }}" readonly>
                                <input type="text" class="form-control" name="supplier_nama" id="supplier_nama" value="{{ $data->supplier_nama }}">
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-lg-10">
                                <input type="text" class="form-control" name="supplier_alamat" id="supplier_alamat" value="{{ $data->supplier_alamat }}">
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-lg-10">
                                <select name="supplier_komoditas[]" id="supplier_komoditas" multiple="multiple" class="form-control select" data-fouc data-placeholder="Pilih Komoditas...">
                                    @foreach ($produk as $r)
                                        <option value="{{ $r->komoditas_id }}" {{ in_array($r->komoditas_id, $selected_produk) ? 'selected' : '' }}>{{ $r->komoditas_nama }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
...
<script>
    function simpan_data(){
        $.ajax({
            type: "PUT",
            url: "{{ route('masterdata.mst_supplier.update', ':id') }}".replace(':id', {{ $data->supplier_id }}),
